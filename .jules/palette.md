...
## 2026-07-03 - Mobile Keyboard and Password Manager UX
**Learning:** Authentication forms that lack the `autoComplete`, `autoCapitalize`, `autoCorrect`, and `spellCheck` attributes cause significant friction on mobile devices where auto-capitalization and autocorrect often interfere with typing email addresses. Furthermore, password managers rely on `autoComplete="current-password"` or `"new-password"` to know when to prompt for saving or generating a password.
