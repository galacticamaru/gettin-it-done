...
## 2024-05-19 - Improved Auth Form Accessibility and Mobile UX
**Learning:** Adding correct `autoComplete` attributes (and disabling auto-capitalization/spellcheck for emails) to authentication inputs significantly enhances the mobile experience by preventing frustrating keyboard behaviors and enabling seamless password manager integration.
**Action:** Always include complete autocomplete properties for inputs that capture standard user information (emails, passwords, names). Ensure `isSignUp` logic correctly toggles between `new-password` and `current-password`.
