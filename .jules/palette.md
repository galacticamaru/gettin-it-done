...
## 2024-06-26 - Optimized auth input fields for mobile and password managers
**Learning:** Auth forms missing explicit attributes like `autoComplete`, `autoCapitalize`, and `spellCheck` can lead to frustrating experiences on mobile keyboards (like capitalizing emails) and prevent password managers from accurately identifying input fields.
**Action:** Always add standard HTML attributes (`autoComplete="email"`, `autoCapitalize="none"`, `autoCorrect="off"`, `spellCheck={false}`) to email fields, and conditional `autoComplete` (`new-password` / `current-password`) to password fields to ensure proper integration with system keyboards and password managers.
