...
## 2026-06-14 - Mobile Keyboard and Password Manager Form Optimizations
**Learning:** Forms utilizing standard email and password inputs often lack HTML attributes that signal context to mobile keyboards and password managers. This leads to frustrating experiences where mobile keyboards try to autocorrect email addresses, or password managers fail to distinguish between account creation and login forms. Furthermore, failing to disable form toggle buttons (like 'Sign in' vs 'Sign up' switchers) during async submissions can cause inconsistent state if users click them rapidly.
**Action:** When implementing authentication forms, always add `autoComplete="email"`, `autoCapitalize="none"`, `autoCorrect="off"`, and `spellCheck={false}` to email inputs. For password inputs, conditionally apply `autoComplete="new-password"` or `autoComplete="current-password"` based on the form's mode. Additionally, ensure all interactive elements within the form context, including mode toggles and visibility switches, are explicitly disabled and visually dimmed (`disabled:opacity-50`) during loading states.
