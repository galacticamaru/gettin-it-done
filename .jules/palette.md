...
## 2026-05-18 - Authentication Password Visibility and Autofill
**Learning:** For authentication forms, adding standard HTML attributes like `autoComplete="email"`, `autoCapitalize="none"`, `autoCorrect="off"`, and `spellCheck={false}` to email inputs prevents mobile autocorrect from interfering with entry. Furthermore, dynamically toggling `autoComplete` between `new-password` and `current-password` allows password managers to correctly identify the context (registration vs login) and prompt the user accordingly. This represents a huge quality-of-life win, especially on mobile.
**Action:** Whenever implementing authentication forms, always apply these explicit auto-fill and correction attributes to optimize for mobile keyboards and enable smooth integration with password managers.
