...
## 2026-06-05 - Tooltip Delay Consistency
**Learning:** Radix UI's `TooltipProvider` has a default `delayDuration` of 700ms. In a highly interactive app, this can feel sluggish. When tooltips are used extensively (e.g., in `DesktopTaskInput`, `TaskOptionsModal`, `SettingsDrawer`, etc.), leaving this unconfigured leads to inconsistent and slow perceived responsiveness.
**Action:** When using Radix UI / shadcn `TooltipProvider` components throughout the application, explicitly set the `delayDuration` prop (e.g., `delayDuration={500}`) to override the default and ensure a consistent, more responsive user experience.
