...
**Learning:** Dynamic lists where items can be added, deleted, completed, or filtered without page reloads (like the task list) need a way to announce these changes to screen readers.
**Action:** Always add `aria-live="polite"` to the wrapper elements of dynamic lists to ensure accessibility for screen reader users, so changes are announced without interrupting the user's current task.
