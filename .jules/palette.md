...
## 2026-06-19 - Auth Form Inputs for Mobile Keyboards and Password Managers
**Learning:** For authentication forms, not having `autoComplete`, `autoCapitalize`, `autoCorrect`, and `spellCheck` attributes on email inputs and the correct `autoComplete` attribute on password inputs creates friction for users, especially on mobile. Mobile keyboards might capitalize the first letter of the email, try to spell check it, or auto-correct it. Password managers might not know if it's a new or existing password field.
**Action:** Always add `autoComplete="email"`, `autoCapitalize="none"`, `autoCorrect="off"`, and `spellCheck={false}` to email inputs, and appropriate `autoComplete` ('new-password' or 'current-password') to password inputs to optimize for mobile keyboards and enable smooth integration with password managers.
