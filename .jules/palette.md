...
## 2026-05-26 - Keyboard Navigation Hints
**Learning:** Adding keyboard shortcuts (like `Escape` to clear an input) greatly enhances power-user efficiency, but these shortcuts remain undiscoverable unless explicitly documented. Adding styled `<kbd>` tags within tooltips is a fantastic way to passively educate users on available shortcuts without cluttering the visible UI.
**Action:** When implementing hidden keyboard shortcuts (like `Escape` or `Enter`), always ensure they are surfaced to the user via tooltips on the corresponding action buttons. Use consistent `<kbd>` styling for the key names to make them visually distinct from normal text.
