...
## 2026-06-13 - aria-live Announcements for Dynamic Task Lists
**Learning:** When creating dynamic lists (like task lists) where items can be added, deleted, completed, or filtered without page reloads, the changes are often not announced to screen reader users by default, leaving them unaware of updates. Adding `aria-live="polite"` to the wrapper element ensures these dynamic updates are politely announced.
**Action:** Always add `aria-live="polite"` to the wrapper elements of dynamic, client-side rendered lists to ensure accessibility for screen reader users.
