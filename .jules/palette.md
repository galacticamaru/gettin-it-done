...
## 2026-06-03 - Authentication Form AutoComplete Attributes
**Learning:** Email and password fields in authentication forms frequently lack proper `autoComplete`, `autoCapitalize`, `autoCorrect`, and `spellCheck` attributes. This causes friction for mobile users (e.g., auto-capitalizing the first letter of an email) and prevents password managers from seamlessly identifying and filling credentials (especially distinguishing between 'new-password' for signup and 'current-password' for login).
**Action:** For authentication forms, always add `autoComplete="email"`, `autoCapitalize="none"`, `autoCorrect="off"`, and `spellCheck={false}` to email inputs, and appropriate `autoComplete` ('new-password' or 'current-password') to password inputs to optimize for mobile keyboards and enable smooth integration with password managers.
