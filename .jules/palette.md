...
## 2024-05-31 - Mobile Keyboard Optimization for Auth Forms\n**Learning:** Relying purely on the `type` attribute (like `type="email"`) is often insufficient to prevent mobile keyboards from applying frustrating defaults like auto-capitalizing the first letter or attempting to auto-correct email addresses. Furthermore, password inputs benefit greatly from explicit hints about whether they expect a *new* password (during sign-up) or an *existing* password (during sign-in) to interface correctly with password managers.\n**Action:** When building authentication forms, always apply `autoComplete="email"`, `autoCapitalize="none"`, `autoCorrect="off"`, and `spellCheck={false}` to email inputs. For password fields, dynamically apply `autoComplete="new-password"` or `autoComplete="current-password"` based on the context of the form to optimize the user experience.
